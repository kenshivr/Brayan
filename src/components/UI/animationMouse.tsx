...
function useFancyCursor() {
  ...
  useEffect(() => {
    ...
    const onLeave = () =>
      setState((s) => ({ ...s, hidden: true, hover: false }));
...
const AnimationMouse: React.FC = () => {
  ...
  return (
    <>
      {/* DOT */}
      <div
        className="pointer-events-none fixed top-0 left-0 z-[9999] rounded-full transition-transform duration-150 ease-out will-change-transform"
        style={{
          width: dotSize,
          height: dotSize,
      ...
      {/* RING */}
      <div
        className={`pointer-events-none fixed top-0 left-0 z-[9998] rounded-full transition-[transform,opacity] duration-200 ease-out will-change-transform ${baseBlend}`}
        style={{
          width: ringSize,
          height: ringSize,
          transform: `translate(${ringX - ringSize / 2}px, ${
            ringY - ringSize / 2
          }px) scale(${ringScale})`,
          opacity: hidden ? 0 : 1,
          border: "1px solid var(--color-firstColor)",
        }}
      />
      {/* HALO */}
      <div
        className="pointer-events-none fixed top-0 left-0 z-[9997] rounded-full blur-2xl transition-opacity duration-300 will-change-transform"
        style={{
          backgroundColor: "var(--color-firstColor)",
          width: ringSize * haloScale,
          height: ringSize * haloScale,
          transform: `translate(${ringX - (ringSize * haloScale) / 2}px, ${
            ringY - (ringSize * haloScale) / 2
          }px)`,
